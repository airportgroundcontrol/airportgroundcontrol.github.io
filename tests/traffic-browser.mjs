...
const action = label => page.getByRole('menuitem', { name: label, exact: false }).click();
const advance = async (id, condition) => {
  const result = await page.evaluate(({ id, condition }) => {
    const s = groundControl.sim, p = s.planes.find(p => p.id === id);
    const done = () => condition === 'holdReached' ? p.holdReached : p.state === condition;
    for (let i = 0; i < 10000 && !done(); i++) s.tick(.1);
    return !!done();
  }, { id, condition });
  assert.ok(result);
};

await page.waitForFunction(() => document.querySelectorAll('.request-group').length === 2);
assert.equal(await page.getByRole('region', { name: 'Request pushback', exact: true }).locator('.strip').count(), 3);
assert.equal(await page.locator('.strip-status').count(), 0);
assert.equal(await page.locator('.strip.request').count(), 4);
assert.equal(await page.locator('.strip').first().innerText(), 'BAW1439\nA320 / S3');
assert.equal(await page.locator('.strip.request').first().evaluate(el => getComputedStyle(el, '::after').animationName), 'request-pulse');
...
await page.waitForFunction(() => [...document.querySelectorAll('.flight-group h3')].some(el => el.textContent.includes('Request onward taxi')));
await select('BAW1439'); await page.keyboard.press('t'); await page.keyboard.press('Enter');
await page.keyboard.press('s');
const holdOption = await page.getByLabel('Hold short of', { exact: true }).locator('option:not(:disabled)').first().getAttribute('value');
await page.getByLabel('Hold short of', { exact: true }).selectOption(holdOption);
...
await page.evaluate(() => {
  const s = groundControl.sim; s.reset(); s.nextArrival = s.nextDeparture = Infinity;
  for (const id of [2, 1]) { s.command(id, 'pushback'); for (let i = 0; i < 2000 && s.planes.find(p => p.id === id).state !== 'ready'; i++) s.tick(.1); }
  s.command(2, 'taxi'); for (let i = 0; i < 250; i++) s.tick(.1); s.command(1, 'taxi');
});
await select('BAW1439'); await page.keyboard.press('y');
assert.ok(await page.getByLabel('Follow aircraft', { exact: true }).locator('option').count());
await page.getByLabel('Follow aircraft', { exact: true }).selectOption('2');
await action('Issue instruction');
assert.equal(await page.evaluate(() => groundControl.sim.planes[0].trafficOrder.kind), 'follow');
assert.equal(await page.locator('#aircraft-menu').isVisible(), false);
await select('BAW1439'); await page.keyboard.press('x');
assert.equal(await page.evaluate(() => groundControl.sim.planes[0].trafficOrder), null);
await select('BAW1439'); await page.keyboard.press('w');
await page.getByLabel('Give way to aircraft', { exact: true }).selectOption('2');
await page.locator('#aircraft-menu').focus(); await page.keyboard.press('Enter');
assert.equal(await page.evaluate(() => groundControl.sim.planes[0].trafficOrder.kind), 'giveway');
assert.equal(await page.locator('#aircraft-menu').isVisible(), false);
await page.waitForFunction(() => !document.querySelector('.strip[data-flight="1"]').classList.contains('request'));
await select('BAW1439'); await page.keyboard.press('y');
await page.evaluate(() => { groundControl.sim.planes.find(p => p.id === 2).state = 'done'; });
await page.waitForFunction(() => document.querySelector('[data-action="confirm"]')?.disabled);
await page.keyboard.press('Escape');
await page.evaluate(() => { groundControl.sim.planes.find(p => p.id === 2).state = 'taxi'; });
