...
await page.evaluate(() => {
  ...
  s.command(1, 'pushback'); for (let i = 0; i < 500 && s.planes[0].state !== 'ready'; i++) s.tick(.1);
  s.command(1, 'taxi'); s.command(1, 'holdshort', { holdPoint: s.holdOptions(s.planes[0])[0].id });
  s.command(4, 'land'); for (let i = 0; i < 30; i++) s.tick(.1);
  s.score = 250; s.completed = 3; s.incidents = 2; s.conflictPairs.add('1:2');
});
...
await page.evaluate(() => { groundControl.map.camera = { x: 125, y: -240, zoom: .6 }; });
const snapshot = () => page.evaluate(() => {
  const s = groundControl.sim;
  return { time: s.time, score: s.score, completed: s.completed, incidents: s.incidents, runway: s.runwayOwner, planes: s.planes, logs: s.logs, conflicts: [...s.conflictPairs], nextId: s.nextId, nextArrival: s.nextArrival, nextDeparture: s.nextDeparture, nextCleanup: s.nextCleanup, camera: groundControl.map.camera };
});
const before = await snapshot();
await page.reload(); await page.waitForFunction(() => window.groundControl);
assert.deepEqual(await snapshot(), before);
assert.equal(await page.locator('#simulation-status').textContent(), 'PAUSED');
assert.equal(await page.locator('[data-speed="8"]').getAttribute('class'), 'active');
assert.equal(await page.locator('[data-filter="departure"]').getAttribute('class'), 'active');
...
await page.evaluate(() => { groundControl.sim.score = 345; });
...
await reopened.evaluate(() => {
  groundControl.sim.command(1, 'hold'); groundControl.select(1); groundControl.preview();
});
...
await reopened.reload(); await reopened.waitForFunction(() => window.groundControl);
assert.deepEqual(await reopened.evaluate(() => groundControl.map.preview), draft);
...
await reopened.getByRole('button', { name: 'Keyboard shortcuts', exact: true }).click();
assert.equal(await reopened.locator('#simulation-status').textContent(), 'PAUSED');
await reopened.reload(); await reopened.waitForFunction(() => window.groundControl);
assert.equal(await reopened.locator('#simulation-status').textContent(), 'RUNNING');
await reopened.evaluate(() => groundControl.setPaused(true));

await reopened.getByRole('button', { name: 'Restart simulation', exact: true }).click();
...
assert.equal(await reopened.evaluate(() => groundControl.sim.planes.length), 4);
...
assert.equal(await blocked.evaluate(() => groundControl.sim.planes[0].state), 'pushback');
...
await offline.evaluate(() => { groundControl.sim.score = 777; groundControl.setPaused(true); });
await offline.reload(); await offline.waitForFunction(() => window.groundControl);
