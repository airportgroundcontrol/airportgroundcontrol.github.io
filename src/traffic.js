...
export function segments(plane, horizon = Infinity) {
  const result = [];
  let start = plane, travelled = 0;
  for (const end of plane.route) {
    const size = length(start, end);
    if (size > .001) result.push({ start, end, size, travelled });
    travelled += size;
    start = end;
...
export function distanceAhead(plane, point, corridor = 24, horizon = 300) {
  for (const segment of segments(plane, horizon)) {
    const v = sub(segment.end, segment.start), offset = sub(point, segment.start);
    const t = dot(v, offset) / (segment.size * segment.size);
    if (t < 0 || t > 1 || Math.abs(cross(v, offset)) / segment.size > corridor) continue;
    const distance = segment.travelled + t * segment.size;
    if (distance > .01 && distance <= horizon) return distance;
  }
  return Infinity;
}

// Intersections include collinear overlaps, so merging and same-taxiway queues share one model.
export function routeConflict(a, b, { aligned = false, horizon = Infinity } = {}) {
  let first = null;
  for (const sa of segments(a, horizon)) for (const sb of segments(b, horizon)) {
    const r = sub(sa.end, sa.start), s = sub(sb.end, sb.start), offset = sub(sb.start, sa.start);
    const denominator = cross(r, s), alignment = dot(r, s) / (sa.size * sb.size);
    if (aligned && alignment < .85) continue;
    let t, u;
    if (Math.abs(denominator) < .0001) {
      if (Math.abs(cross(offset, r)) / sa.size > .1) continue;
      const t0 = dot(offset, r) / dot(r, r), t1 = t0 + dot(s, r) / dot(r, r);
      t = Math.max(0, Math.min(t0, t1));
      if (t > Math.min(1, Math.max(t0, t1)) + .00001) continue;
    ...
    const distanceA = sa.travelled + t * sa.size, distanceB = sb.travelled + u * sb.size;
    if (distanceA > horizon || distanceB > horizon) continue;
    if (!first || distanceA < first.distanceA || (distanceA === first.distanceA && distanceB < first.distanceB)) {
      first = { point: { x: sa.start.x + t * r.x, y: sa.start.y + t * r.y }, distanceA, distanceB, alignment, heading: r };
    }
  }
  return first;
}
