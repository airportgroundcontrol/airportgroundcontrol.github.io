...
const server = createServer(async (request, response) => {
  ...
  try {
    const body = await readFile(
      new URL("../dist/" + entry[0], import.meta.url),
    );
    response
      .writeHead(200, { "Content-Type": entry[1], "Cache-Control": "no-store" })
      .end(body);
  } catch {
    response.writeHead(500).end("Build the site before running browser tests.");
  }
});
let child;
